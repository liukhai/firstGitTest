...
@implementation TaxLoanTNCViewController
@synthesize webView;
...
//    bgv.frame = CGRectMake(0, 0, 320, 367+[[MyScreenUtil me] getScreenHeightAdjust]);
	...
	//NSString *path = [[NSBundle mainBundle] pathForResource:@"TNC_zh" ofType:@"htm"];
    ...
    [self.webView loadRequest:[HttpRequestUtils getPostRequest_loanTNC]];
...
-(void) webViewDidStartLoad:(UIWebView *)webView {
	NSLog(@"Start load TaxLoanTNCViewController");
	[[CoreData sharedCoreData].mask showMask];
}

-(void) webViewDidFinishLoad:(UIWebView *)webView {
	NSLog(@"Finish loaded TaxLoanTNCViewController");
	[[CoreData sharedCoreData].mask hiddenMask];
}

-(void) webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
	[[CoreData sharedCoreData].mask hiddenMask];
	NSLog(@"fail loaded TaxLoanTNCViewController" );
...
//	[CoreData sharedCoreData].taxLoan_view_controller.view.center = [[MyScreenUtil me] getmainScreenRight:self];
    ...
    NSLog(@"Navigation Revamp - Consumer Loan - TaxLoanTNCViewController.m");
