...
@implementation TaxLoanViewController
...
-(void) navigationController:(UINavigationController *)pnavigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if ([viewController class]==[TaxLoanViewController class]) {
//        [UIView beginAnimations:nil context:NULL];
//        [UIView setAnimationDuration:0.5];
...
-(void)goMain
{
    ...
    [CoreData sharedCoreData].bea_view_controller.vc4process = nil;
    NSLog(@"TaxLoanViewController goMain:%@",[CoreData sharedCoreData].bea_view_controller.vc4process);
}
...
//    [self forwardNextView:NSClassFromString(@"ConsumerLoanListViewController") viewName:@"ConsumerLoanListViewController"];
    menuVC = [[ConsumerLoanMenuViewController alloc] initWithNibName:@"ConsumerLoanMenuViewController" bundle:nil nav:self.navigationController];
//    [self.navigationController popToRootViewControllerAnimated:FALSE];
