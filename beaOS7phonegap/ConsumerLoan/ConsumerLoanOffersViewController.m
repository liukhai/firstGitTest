//
//  ConsumerLoanOffersViewController.m
//  BEA
//
...
@implementation ConsumerLoanOffersViewController

@synthesize webView;
@synthesize consumerLoanPromoUrl, consumerLoanPromoHotline, consumerLoanBtnLabel;
@synthesize consumerLoanTNCUrl;
//@synthesize callwebView;
@synthesize url2, url2label, btnGotoURL, tnc_label;
...
//    btnBookmark.frame = CGRectMake(btnBookmark.frame.origin, 75, 25, 25);
    ...
    if ([pageTheme isEqualToString:@"1"]) {
        ...
        else if([bookmark_data isOfferExist:merchant_info InGroup:8] && [fromType isEqualToString:@"CL"]) {
            [btnBookmark setBackgroundImage:[UIImage imageNamed:@"btn_bookmark_on.png"] forState:UIControlStateNormal];
        }
    } else {
        if ([bookmark_data isOfferExist:merchant_info InGroup:1] && [fromType isEqualToString:@"Pri"]) {
...
{
    if (mfirst) {
        if ([fromType isEqualToString:@"CL"]) {
            NSMutableArray *viewControllers = [NSMutableArray arrayWithArray:[[CoreData sharedCoreData].taxLoan_view_controller.navigationController viewControllers]];
            [viewControllers removeLastObject];
            if ([[viewControllers lastObject] isKindOfClass:[ConsumerLoanMenuViewController class]]) {
                [[CoreData sharedCoreData].taxLoan_view_controller.navigationController popViewControllerAnimated:NO];
                [(ConsumerLoanMenuViewController *)[viewControllers lastObject] welcome:show+1];
            }
        }
...
//        NSLog(@"desc is %@", [merchant_info objectForKey:@"desc"]);
...
//        [descLabel setFrame:CGRectMake(10.0, size.height + 10, _contentScroll.frame.size.width - 20, labelsize.height)];
        ...
        descLabel.frame = CGRectMake(descLabel.frame.origin.x, descLabel.frame.origin.y, descLabel.frame.size.width, text_area.height+10);
...
//                subject = [NSString stringWithFormat:@"%@ %@", functionName, appString];
                ...
                if ([merchant_info objectForKey:@"emailsubject"] == nil || [[merchant_info objectForKey:@"emailsubject"] isEqualToString:@""]) {
                    
                    subject = [NSString stringWithFormat:@"%@",NSLocalizedString(@"accpro.offers.EmailTitle", nil)];
                }
            }
            else {
                ...
                if ([fromType isEqualToString:@"PRI"] || fromType == nil) {
                    //                functionName = NSLocalizedString(@"tag_fav_privileges", nil);
                    //                subject = [NSString stringWithFormat:@"%@ %@", functionName, appString];
                    subject = [merchant_info objectForKey:@"emailsubject"];
                    if ([merchant_info objectForKey:@"emailsubject"] == nil || [[merchant_info objectForKey:@"emailsubject"] isEqualToString:@""] || [[merchant_info objectForKey:@"emailsubject"] isEqualToString:@"\n\t\t\t"]) {
                        
                        subject = [NSString stringWithFormat:@"%@",NSLocalizedString(@"accpro.offers.EmailTitle", nil)];
                    }
                    NSLog(@"subject is %@", subject);
                }else if (oldSubject == nil || [oldSubject isEqualToString:@""] || [oldSubject isEqualToString:@"\n\t\t\t"]) {
                    oldSubject = [NSString stringWithFormat:@"%@",NSLocalizedString(@"consumerloan.offers.EmailTitle", nil)];
                    subject = [oldSubject stringByReplacingOccurrencesOfString:@"BEA Consumer Loan" withString:@"BEA Consumer Loans"];
                }
...
- (void)doBookmark{
    ...
    if ([fromType isEqualToString:@"Pri"]) {
        ...
        if ([bookmark_data isOfferExist:merchant_info InGroup:8]) {
            [bookmark_data removeBookmark:merchant_info InGroup:8];
            if ([pageTheme isEqualToString:@"1"]) {
                [btnBookmark setBackgroundImage:[UIImage imageNamed:@"btn_bookmark.png"] forState:UIControlStateNormal];
            } else {
                [btnBookmark setBackgroundImage:[UIImage imageNamed:@"btn_bookmark_new.png"] forState:UIControlStateNormal];
            ...
            [bookmark_data addBookmark:merchant_info ToGroup:8];
            if ([pageTheme isEqualToString:@"1"]) {
                [btnBookmark setBackgroundImage:[UIImage imageNamed:@"btn_bookmark_on.png"] forState:UIControlStateNormal];
            } else {
                [btnBookmark setBackgroundImage:[UIImage imageNamed:@"btn_bookmark_on_new.png"] forState:UIControlStateNormal];
