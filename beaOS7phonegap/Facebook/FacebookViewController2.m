//
//  FacebookViewController2.m
//  BEA
//
//  Created by Helen on 14-8-26.
//  Copyright (c) 2014年 The Bank of East Asia, Limited. All rights reserved.
//

#import "FacebookViewController2.h"

@interface FacebookViewController2 ()

@end

@implementation FacebookViewController2
@synthesize v_rmvc;
...
- (void)setUrlRequest:(NSURLRequest *)urlrequest setTitle:(NSString *)title{
    ...
    NSLog(@"debug FacebookViewController2 setUrlRequest:%@", urlRequest);
}
...
- (void)viewDidLoad
{
    ...
    NSURL *url= [NSURL URLWithString:[NSString stringWithFormat:@"%@",[MigrationSetting me].URLOfFacebook_joy]];
...
-(void) webViewDidStartLoad:(UIWebView *)webView {
	NSLog(@"Start load FacebookViewController2");
	[[CoreData sharedCoreData].mask showMask];
}

-(void) webViewDidFinishLoad:(UIWebView *)webView {
	NSLog(@"Finish loaded FacebookViewController2");
...
-(void) webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    [[CoreData sharedCoreData].mask hiddenMask];
    NSLog(@"fail loaded FacebookViewController2:%@", error );
...
{
    NSLog(@"FacebookViewController2 mainDocumentURL:%@", request.mainDocumentURL );
    return YES;
}
