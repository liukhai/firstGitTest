...
@implementation CardLoanListViewController
...
//edit by chu 20150213
- (void)viewWillAppear:(BOOL)animated{
    [super viewWillAppear:animated];
    
    title_label.text = [NSString stringWithFormat:@"%@",NSLocalizedString(@"tag_fav_cc_cash_in_hand",nil)];
    title_label.accessibilityLabel = [NSString stringWithFormat:@"%@",NSLocalizedString(@"tag_fav_cc_cash_in_hand",nil)];
}
...
-(void)getItemsList {
	...
	asi_request = [[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@latestpromotions.api?type=CIH&lang=%@&UDID=%@",[CoreData sharedCoreData].realServerURLCard,[CoreData sharedCoreData].lang,[CoreData sharedCoreData].UDID]]];
	asi_request.delegate = self;
	[[CoreData sharedCoreData].queue addOperation:asi_request];
	
	//jeff
	NSLog(@"class: %@, asi_request.url: %@", [self class], asi_request.url);
	//
}
...
//    NSLog(@"debug CardLoanListViewController requestFinished:%@",[request responseString]);
...
- (void)requestFailed:(ASIHTTPRequest *)request {
    NSLog(@"CardLoanListViewController requestFailed:%@", [request error]);
...
-(NSInteger) tableView:(UITableView *)table numberOfRowsInSection:(NSInteger)section {
	//jeff-exchange first row and second row
	/*
	if([items_data count] > 1){
		NSMutableDictionary *temp_dict0 = [[items_data objectAtIndex:0] copy];
		[items_data replaceObjectAtIndex:0 withObject:[items_data objectAtIndex:1]];
		[items_data replaceObjectAtIndex:1 withObject:temp_dict0];
		if(temp_dict0 != nil){
			[temp_dict0 release];
			temp_dict0 = nil;
		}
...
//        
//    } else {
        if (indexPath.row == [items_data count] -1 ) {
            if ([items_data count] > 5) {
...
-(void) parserDidStartDocument:(NSXMLParser *)parser {
	[items_data removeAllObjects];
	current_page = 1;
	key = [NSArray arrayWithObjects:@"id",@"title",@"etitle",@"shortdescription",@"description",@"thumbnail",@"category",@"preview",@"cuisine",@"suprise",@"address",@"tel",@"card",@"coupon",@"remark",@"newitem", @"cashinhandtype",nil];
}
