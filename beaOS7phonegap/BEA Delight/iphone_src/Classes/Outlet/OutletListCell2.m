...
@implementation OutletListCell2
@synthesize address, tel, opening, distance,handset;
...
//		handset = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_telephone.png"]];
		...
		//jeff
		address = [[UILabel alloc] initWithFrame:CGRectMake(35, 8, 225, 45)];
		//
		...
		//jeff
		tel = [[UIButton alloc] initWithFrame:CGRectMake(35, 53, 250, 20)];
		//
...
- (void)layoutViews {
    int addressH = [self fitHeightWithString:address.text];
    if (addressH > 45) {
        CGRect frame = address.frame;
        frame.size.height = addressH;
        address.frame = frame;
        handset.center = CGPointMake(20, 64 + addressH - 45);
        frame = tel.frame;
        frame.origin.y = 53 + addressH - 45;
        tel.frame = frame;
    }
}
...
-(void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	if (buttonIndex==1) {
        ...
        NSArray *telArray = [[tel titleForState:UIControlStateNormal] componentsSeparatedByString:@"/"];
		[[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"tel:%@",[[telArray firstObject] stringByReplacingOccurrencesOfString:@" " withString:@""]]]];
	}
}
