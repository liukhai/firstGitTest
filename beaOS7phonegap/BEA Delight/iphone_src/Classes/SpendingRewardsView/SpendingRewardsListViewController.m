//
//  SpendingRewardsListViewController.m
//  BEA Surprise
//
...
@implementation SpendingRewardsListViewController
...
//    bgv.frame = CGRectMake(0, 0, 320, 367+[[MyScreenUtil me] getScreenHeightAdjust]);
	...
	title_label.text = [NSString stringWithFormat:@"%@",NSLocalizedString(@"Spending & Rewards",nil)];
...
-(void)initTableView{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
        ...
        //    frame1.size.height = frame1.size.height-22;
...
-(void)getItemsList {
	NSLog(@"Get the list of Spending & Rewards");
	[[CoreData sharedCoreData].mask showMask];
	asi_request = [[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@spending.api?lang=%@&UDID=%@",[CoreData sharedCoreData].realServerURLCard,[CoreData sharedCoreData].lang,[CoreData sharedCoreData].UDID]]];
...
- (void)requestFailed:(ASIHTTPRequest *)request {
    NSLog(@"debug SpendingRewardsListViewController requestFailed:%@", request.error);
