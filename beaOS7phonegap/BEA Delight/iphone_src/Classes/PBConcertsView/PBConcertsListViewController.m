//
//  PBConcertsListViewController.m
//  BEA Surprise
//
...
@implementation PBConcertsListViewController
...
//    table_view.frame = CGRectMake(0, 63, 320, 397+[[MyScreenUtil me] getScreenHeightAdjust]);
	...
	//jeff
	self.title_label.text = [NSString stringWithFormat:@"%@",NSLocalizedString(@"Concert and Movie Offers", nil)];
	//self.title_label.text = NSLocalizedString(@"Concert and Movie Offers", nil);
	//
...
-(void)getItemsList {
	...
	asi_request = [[ASIHTTPRequest alloc] initWithURL:
                   [NSURL URLWithString:
                    [NSString stringWithFormat:@"%@pbconcert.api?lang=%@&UDID=%@",
	...
	//jeff
	NSLog(@"debug PBConcertsListViewController url:%@", asi_request.url);
	//
}
...
//    }
    ...
    //	}
	cell.title_label.text = [[[self.items_data objectAtIndex:indexPath.row + (current_page-1) * current_page_size] objectForKey:@"shortdescription"] stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
	...
	//if (![[[self.items_data objectAtIndex:indexPath.row + (current_page-1) * current_page_size] objectForKey:@"card"] isEqualToString:@"Core"]) {
...
//    if (indexPath.row>0) {
//        if ([CoreData sharedCoreData].bea_view_controller->jump3) {
//            [CoreData sharedCoreData].bea_view_controller->jump3 = NO;
...
-(void) parserDidStartDocument:(NSXMLParser *)parser {
	[self.items_data removeAllObjects];
	current_page = 1;
	//key = [NSArray arrayWithObjects:@"id",@"title",@"etitle",@"shortdescription",@"description",@"thumbnail",@"category",@"preview",@"cuisine",@"suprise",@"card",@"concert_date",@"book_hotline",@"start",@"expire",@"can_book",@"venue",@"price",@"remark",@"newitem",nil];
	//jeff
	key = [NSArray arrayWithObjects:@"id",@"title",@"etitle",@"shortdescription",@"description",@"thumbnail",@"category",@"preview",@"cuisine",@"suprise",@"card",@"concert_date",@"displayhotline",@"book_hotline",@"show_date",@"start",@"expire",@"can_book", @"website", @"website_start", @"website_expire", @"can_websurf",@"venue",@"price",@"remark",@"newitem",nil];
}
...
-(void) parserDidEndDocument:(NSXMLParser *)parser {
	...
	NSLog(@"%d merchants",[self.items_data count]);
    ...
    [[CoreData sharedCoreData].bea_view_controller setAccessibilityForPlistvc3:[[self.items_data objectAtIndex:0] objectForKey:@"title"]];
}
...
-(void) parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
	if ([elementName isEqualToString:@"item"]) {
		...
		/*[temp_record setValue:temp_merchant_list forKey:@"coorganisers"];
		[temp_record setValue:temp_merchant_list forKey:@"sponsors"];
		[temp_record setValue:temp_mgt_list forKey:@"managements"];
		[temp_record setValue:temp_merchant_list forKey:@"productions"];*/
	} else if ([elementName isEqualToString:@"coorganisers"]) {
