//
//  SupremeGoldEnquiryViewController.m
//  BEA
//
...
@implementation SupremeGoldEnquiryViewController
...
-(void)requestDatas {
    NSURL* url= [NSURL URLWithString:[NSString stringWithFormat:@"%@getenquiry.api?type=7&lang=%@",
...
-(void)initView
{
    ...
    for (int i=0; i<[items_data count]; i++) {
        ...
        ecvc.nsSubject = NSLocalizedString(@"supremegold.enquires.EmailTitle",nil);
        ecvc.navvc = [CoreData sharedCoreData].sg_view_controller;
        ...
        height += ecvc.view.frame.size.height + 40;
    }
    
    [contentView setContentSize:CGSizeMake(contentView.frame.size.width, height+20)];
...
-(IBAction)email{
	...
	NSString* subject = [NSString stringWithFormat:@"%@", NSLocalizedString(@"supremegold.enquires.EmailTitle",nil)];
	[mail_controller setToRecipients:to];
	[mail_controller setSubject:subject];
...
-(void)mailComposeController:(MFMailComposeViewController *)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError *)error {
	...
	[SupremeGoldUtil me].SupremeGold_view_controller.tabBar.hidden = NO;
