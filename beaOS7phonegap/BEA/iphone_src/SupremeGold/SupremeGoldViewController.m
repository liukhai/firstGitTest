...
// private method
@interface SupremeGoldViewController ()
...
@implementation SupremeGoldViewController
...
-(void) navigationController:(UINavigationController *)pnavigationController didShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if ([viewController class] == [AccProListViewController class ] || [viewController class] == [SupremeGoldOffersViewController class ]) {
...
-(void) navigationController:(UINavigationController *)pnavigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if ([viewController class]==[RootViewController class]) {//added by jasen on 20111118
        [[SupremeGoldUtil me] backToFromPage];
    }
    
    if ([viewController class]==[SupremeGoldApplicationViewController class]) {
              ...
              tnc:(NSString*)TNCurl
{
    ...
    [self.navigationController popToRootViewControllerAnimated:FALSE];
    //    UIViewController *current_view_controller = [viewController new];
    SupremeGoldOffersViewController *consumerLoanOffersViewController =
    [[SupremeGoldOffersViewController alloc]
     initWithNibName:@"SupremeGoldOffersViewController"
     bundle:nil
     url:url
     hotline:hotline
     btnLabel:btnLabel
