//
//  SupremeGoldApplicationViewController.m
//  BEA
//
...
@implementation SupremeGoldApplicationViewController
...
//    self.view.frame = CGRectMake(0, 0, 320, 326+[[MyScreenUtil me] getScreenHeightAdjust]);
    webView.frame = CGRectMake(0, 0, 320, 367+[[MyScreenUtil me] getScreenHeightAdjust]);
    
    [self.webView loadRequest:[HttpRequestUtils getPostRequest4SupremeGoldApplicationView]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue < 7.0 && [request.mainDocumentURL.relativeString rangeOfString:@"SGPromoInputShow"].location != NSNotFound && !request.HTTPBody) {
        [self.webView loadRequest:[HttpRequestUtils getPostRequest4SupremeGoldApplicationView]];
        return false;
    }
...
-(void) btInterestRatePressed{
    ...
    [webViewController setNav:[CoreData sharedCoreData].sg_view_controller.navigationController];
	[[CoreData sharedCoreData].sg_view_controller.navigationController pushViewController:webViewController animated:NO];
...
- (void)doMenuButtonsPressed:(id)sender {
    ...
    if (self.isShow && [webView.request.mainDocumentURL.relativeString rangeOfString:@"PromoPickShow"].location != NSNotFound) {
        ...
        [CoreData sharedCoreData].sg_view_controller = nil;
