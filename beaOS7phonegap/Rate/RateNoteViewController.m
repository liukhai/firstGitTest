...
@interface RateNoteViewController() 
...
@implementation RateNoteViewController
...
{
    [super viewDidLoad];
    [[[RateUtil me] Rate_view_controller] tabBar].selectedItem = [[[[RateUtil me] Rate_view_controller] tabBar].items objectAtIndex:0];
//    [self.view addSubview:scroll_view];


//    self.scroll_view.frame = CGRectMake(5, 126, 311, 281+[[MyScreenUtil me] getScreenHeightAdjust]);
...
//    [scroll_view setContentSize:CGSizeMake(311, fund_table.frame.size.height)];
//    [scroll_view addSubview:fund_table];
    self.time_label.text=@"";
    self.lbTitle.text = NSLocalizedString(@"Rate.Note.title",nil);
...
-(void) loadFundData{
    ...
    BOOL dateExpired = [[RateUtil me] checkRateSNExpired:[[RateUtil me] findRatePlistPath:@"note"]];
...
-(void) sendRequest{
    ASIFormDataRequest *request = [HttpRequestUtils getRequestForRatePlist:self rate:@"rnote"];
...
-(void) parseCfgFile{
    NSMutableDictionary *md_temp = [NSMutableDictionary dictionaryWithContentsOfFile:[[RateUtil me ]findRatePlistPath:@"note"]];
    ...
    self.items_data = [[NSMutableArray alloc] initWithArray:[md_temp objectForKey:@"note"] copyItems:YES];
	NSLog(@"RateNoteViewController: parseCfgFile count:%d", [self.items_data count]);
    self.items_total = [[NSMutableArray alloc] initWithArray:[md_temp objectForKey:@"note"] copyItems:YES];
...
//    NSString* reponsedString = [NSString stringWithFormat:@"%@", [request responseString]];
    NSLog(@"RateNote: request finish");
//    NSLog(@"RateNote: request finish:%@", reponsedString);
    [[CoreData sharedCoreData].mask hiddenMask];

    if(![RateUtil checkResponseData:[request responseData] rateType:@"note"]){
        [[RateUtil me] alertAndBackToMain:self]; 
        return;
    }
    [RateUtil updateRateFile:[request responseData] rateType:@"note"];
    [self parseCfgFile];
    [fund_table reloadData];
}

- (void)requestFailed:(ASIHTTPRequest *)request {
    NSLog(@"RateNoteViewController: request failed:%@", request.error);
